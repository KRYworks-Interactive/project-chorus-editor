...
namespace Ched.UI
{
    ...
    public class SoundPreviewManager : IDisposable
    {
        ...
        private LinkedListNode<HighSpeedChangeEvent> HighSpeedElement;
        ...
        public bool Start(ISoundPreviewContext context, int startTick)
        {
            ...
            var speedEvents = context.HighSpeedChangeEvents.ToList();
            if (!speedEvents.Any(p => p.Tick == 0))
            {
                speedEvents.Insert(0, new HighSpeedChangeEvent() { Tick = 0, SpeedRatio = 1.0m });
            }
            HighSpeedElement = new LinkedList<HighSpeedChangeEvent>(speedEvents.OrderBy(p => p.Tick)).First;
            ...
            while (HighSpeedElement.Next != null && HighSpeedElement.Next.Value.Tick <= startTick) HighSpeedElement = HighSpeedElement.Next;
            while (BpmElement.Next != null && BpmElement.Next.Value.Tick <= startTick) BpmElement = BpmElement.Next;

            int clapLatencyTick = GetLatencyTick(ClapSource.Latency, BpmElement.Value.Bpm, HighSpeedElement.Value.SpeedRatio);
            ...
            Task.Run(() =>
            {
                ...
                if (!Playing) return;
                SoundManager.PlayMusic(context.MusicSource.FilePath, startTime + context.MusicSource.Latency, (double)HighSpeedElement.Value.SpeedRatio);
            })
            .ContinueWith(p =>
            {
                if (p.Exception != null)
        ...
        private void Tick(object sender, EventArgs e)
        {
            ...
            elapsedTick += PreviewContext.TicksPerBeat * BpmElement.Value.Bpm * (double)HighSpeedElement.Value.SpeedRatio * elapsed / 60 / 1000;
            CurrentTick = (int)(InitialTick + elapsedTick);
            if (CurrentTick >= StartTick)
                TickUpdated?.Invoke(this, new TickUpdatedEventArgs(Math.Max(CurrentTick, 0)));

            var prevSpeedRatio = HighSpeedElement.Value.SpeedRatio;
            while (HighSpeedElement.Next != null && HighSpeedElement.Next.Value.Tick <= CurrentTick) HighSpeedElement = HighSpeedElement.Next;
            if (prevSpeedRatio != HighSpeedElement.Value.SpeedRatio)
            {
                SoundManager.SetMusicSpeed((double)HighSpeedElement.Value.SpeedRatio);
            }
            ...
            int latencyTick = GetLatencyTick(ClapSource.Latency, BpmElement.Value.Bpm, HighSpeedElement.Value.SpeedRatio);
        ...
        private int GetLatencyTick(double latency, double bpm, decimal speedRatio)
        {
            return (int)(PreviewContext.TicksPerBeat * latency * bpm * (double)speedRatio / 60);
        }
    ...
    public interface ISoundPreviewContext
    {
        ...
        IEnumerable<HighSpeedChangeEvent> HighSpeedChangeEvents { get; }
    ...
    public class SoundPreviewContext : ISoundPreviewContext
    {
        ...
        public IEnumerable<HighSpeedChangeEvent> HighSpeedChangeEvents => score.Events.HighSpeedChangeEvents;
